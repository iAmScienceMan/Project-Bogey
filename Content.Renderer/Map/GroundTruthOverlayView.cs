...
public sealed class GroundTruthOverlayView
{
    ...
    private DisplayMode _mode = DisplayMode.Hidden;
    private int? _selectedEntity;
    private bool _showSeekers;

    public bool WantsData => _mode != DisplayMode.Hidden || _showSeekers;

    public bool IsVisible => _mode != DisplayMode.Hidden;
    ...
    public string SetVisible(bool visible)
    {
        _mode = visible ? DisplayMode.LabelsAndMarkers : DisplayMode.Hidden;
        return ModeName(_mode);
    }
    ...
    public IReadOnlyList<string> DescribeMunitions(IReadOnlyList<MunitionDebugView> munitions)
    {
        List<string> lines = new();
        foreach (MunitionDebugView munition in munitions)
        {
            lines.Add(string.Create(CultureInfo.InvariantCulture,
                $"#{munition.Id} {munition.Side} {munition.Seeker} {PhaseOf(munition)} " +
    ...
    public (int EntityId, Vector2 World)? PickOrPlace(Vector2 screenPx, Camera2D camera, GroundTruthUpdate truth)
    {
        ...
        if (TryPick(screenPx, camera, truth, out int entityId))
        {
        ...
        {
            _selectedEntity = null;
            return (selected, camera.ScreenToWorld(screenPx));
        }

        return null;
    }

    public void Draw(PrimitiveBatch prims, TextBatch text, Camera2D camera, Vector2 viewport, GroundTruthUpdate truth)
    {
        if (_mode == DisplayMode.Hidden && !_showSeekers)
        {
            return;
        }
        ...
        if (_mode != DisplayMode.Hidden)
        {
            ...
            foreach (GroundTruthView entry in truth.Entities)
            {
        ...
        if (_showSeekers)
        {
            DrawSeekers(prims, text, camera, truth.Munitions);
        }

        DrawFooter(text, viewport, selectedName);
    }

    private static void DrawSeekers(PrimitiveBatch prims, TextBatch text, Camera2D camera, IReadOnlyList<MunitionDebugView> munitions)
    {
        foreach (MunitionDebugView munition in munitions)
        {
            Rgba color = ColorFor(munition.Side);
    ...
    private static void DrawSeekerCone(PrimitiveBatch prims, Camera2D camera, MunitionDebugView munition, Rgba color)
    {
    ...
    private static string PhaseOf(MunitionDebugView munition)
    {
        if (munition.Locked)
        {
            return "LOCKED";
    ...
    private static bool TryPick(Vector2 screenPx, Camera2D camera, GroundTruthUpdate truth, out int entityId)
    {
        entityId = 0;
    ...
    private void DrawFooter(TextBatch text, Vector2 viewport, string? selectedName)
    {
        ...
        {
            DisplayMode.LabelsAndMarkers => "labels",
            DisplayMode.MarkersOnly => "markers",
            _ => "hidden",
        };
    ...
    private static string LabelFor(GroundTruthView entry)
    {
